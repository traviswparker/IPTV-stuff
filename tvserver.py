...
BUFFER=1024 #buffer size for streaming
...
def select_acct(accts,print_info=False):
    ...
    print('selected',' '.join(str(x) for x in info[-1][:-1]),file=sys.stderr)
    return info[-1] #account with most available connections

if len(sys.argv) < 2:
    print('''
usage: 
	./tvserver.sh <account_list> 
...
STREAMS=pattern of streams to include regardless of category, matches anywhere in name. !channel to exclude.
STRIP=patterns to strip from channel names. default is anywhere in name, ^pattern for start match, pattern$ for end match
REPLACE=replace any STREAMS with the same name if a channel matching name+pattern exists.
    ...
    def do_GET(self):
        if self.path.startswith('/stream/'):
            ...
            if stream_id in lineup:
                ...
                if int(DIRECT):
                    print('direct from', url, file=sys.stderr)
                    ...
                    print('location:', loc, file=sys.stderr)
                    ...
                    try:
                        ...
                        print('exited:', ffmpeg.returncode, file=sys.stderr)
                    except Exception as e:
            ...
            self.send_header('Content-type', 'application/json')
            self.end_headers()
            self.wfile.write(json.dumps({'ScanInProgress':0,'ScanPossible':1,'Source':'tvserver'}).encode())
                
httpd = http.server.ThreadingHTTPServer((SERVER_IP, int(SERVER_PORT)), HDHR_handler)
print('serving at http://%s:%s' % (SERVER_IP, SERVER_PORT))
httpd.serve_forever()